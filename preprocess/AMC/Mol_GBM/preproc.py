...
def copy_MRI_map(patient_dir):
    ...
    adc_path = os.path.join(map_root, patient_id, 'adc.nii.gz')
    if os.path.exists(adc_path):
        target_path = os.path.join(patient_dir, 'adc.nii.gz')
        os.system(f'fslreorient2std {adc_path} {target_path}')

    dsc_dir = os.path.join(map_root, patient_id, 'output_DSC')
    ...
    dce_files = glob(f'{map_root}/{patient_id}/*Ktrans*.nii')
    if len(dce_files) != 0:
        target_path = os.path.join(patient_dir, 'dce_nordic_1', 'K12.nii.gz')
        os.makedirs(os.path.dirname(target_path), exist_ok=True)
        nifti_map = ants.image_read(dce_files[0], reorient='RPI')
        nifti_map.to_file(target_path)
...
if __name__ == '__main__':
    # source_root = '/data/jhlee/data/AMC/molGBM/nifti_raw'
    ...
    map_root = '/data/aicon/data/AMC/molecularGBM/maps_and_others'
    ...
    # datasets = [mri for mri in datasets if mri.split('/')[-1].split('.nii.gz')[0] not in ['adc', 'dwi_b0', 'swan_p', 'swi_p', 'pcasl', 'dce_moco', 'dsc_moco']]
