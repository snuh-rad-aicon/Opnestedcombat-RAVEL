...
def normalize(patient_dir, target_root, subdir):
    ...
    wm_path = '/mnt/hdd1/shan/GBM/ravel_test/final/wm'
    os.makedirs(target_dir, exist_ok=True)
    wm_mask_file = os.path.join(wm_path, f"{patient_id}_wm_mask.nii.gz")
    modals = ["t1ce", "t1", "t2", "flair", "adc", "rCBV", "K12"]
    wm_mask = nib.load(wm_mask_file).get_fdata().astype(bool)  # Boolean 변환
    ...
    for m in modals:
        try:
            ...
            ravel.fit(array_list, masks=[wm_mask] * len(array_list))
            normalized_images = ravel._normalized
            wm_values = array[wm_mask]
            wm_mean = np.mean(wm_values[wm_values > 0])  # CSF 내 0이 아닌 값만 사용

            # 📌 `vox` 방식으로 정규화 (CSF 평균으로 나누기)
            img_vox = array / wm_mean
...
partial_function = partial(normalize, target_root='/mnt/hdd1/shan/GBM/Ravel_preprocessed/wm/Severance', subdir=True)
